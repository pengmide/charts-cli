...
The QwenPaw `charts` plugin installs the `charts` binary automatically at
startup. Do not install `charts-cli` with npm in the Agent workspace. If the
command is unavailable, report the plugin installation error instead of
